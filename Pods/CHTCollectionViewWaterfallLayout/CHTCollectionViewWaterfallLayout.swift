...
public class CHTCollectionViewWaterfallLayout : UICollectionViewLayout{
        ...
        for section in 0 ..< numberOfSections {
            ...
            */
            ...
            for idx in 0 ..< itemCount {
                ...
                let yOffset = ((self.columnHeights[section] as? NSMutableArray)?.object(at: columnIndex)) as! CGFloat
                let itemSize = self.delegate?.collectionView(collectionView: collectionView!, layout: self, sizeForItemAtIndexPath: indexPath as NSIndexPath)
                var itemHeight : CGFloat = 0.0
                if let height = itemSize?.height, let width =  itemSize?.width {
                    if height > 0 && width > 0 {
                        itemHeight = floor(height*width/width)
                    }
                }
            ...
            */
            ...
            top = CGFloat((((self.columnHeights[section]) as? NSMutableArray)!.object(at: columnIndex)) as! NSInteger) - minimumInteritemSpacing + sectionInsets.bottom
    ...
    //    open var collectionViewContentSize: CGSize { get } // Subclasses must override this method and use it to return the width and height of the collection view’s content. These values represent the width and height of all the content, not just the content that is currently visible. The collection view uses this information to configure its own content size to facilitate scrolling.
